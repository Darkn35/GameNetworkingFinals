...
public class PlayerController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (view.IsMine)
        {
            ...
            if (SceneManager.GetActiveScene().name == "Minigame Level")
            {
                GameObject minigameSpawner = GameObject.FindGameObjectWithTag("MinigameSpawner");
                spawnWalls = minigameSpawner.GetComponent<SpawnWalls>();
                //spawnWalls.view = view;
            }
        }
    }
    ...
    public void EliminatePlayer()
    {
        if (view.IsMine)
        {
            view.RPC("EliminatePlayerRPC", RpcTarget.AllBuffered);
        }
    }
    ...
    private void GrabMechanic(bool isHolding)
    {
        grabMechanic.isHolding = isHolding;

        if (!isHolding)
        {
            grabMechanic.isConnected = isHolding;
            grabMechanic.DestroyHingeJoint();
        }
    }
    ...
    void Update()
    {
        ...
        //if (!IsDead && mIsControlEnabled)
        {
            ...
            {
                _characterController.SimpleMove(Vector3.zero * Time.deltaTime);
            }
        }
    }
    ...
    private void OnControllerColliderHit(ControllerColliderHit hit)
    {
        ...
        //{
        //    if (hit.gameObject.transform.parent.TryGetComponent<WallBehavior>(out WallBehavior wallBehavior))
        //    {
        //        _characterController.Move(wallBehavior.directionOfTravel * 5f);
        //    }
        //}

        //if (hit.gameObject.name != "Platform 1")
        //{
        //    Debug.Log("Collider controller hit" + hit.gameObject.name);
        //}
    }


    public void PushAwayFromWall(Vector3 direction)
    {
        _characterController.Move(direction * 5f);
    }
}
